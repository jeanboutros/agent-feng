...
class TestYamlConfigLoaderLoadConfig:
    ...
    def test_get_property_caches_result(self, tmp_path: Path) -> None:
        ...
        value_1 = loader.get_property("key")
        ...
        value_2 = loader.get_property("key")
        ...
        value_3 = loader.get_property("key")
        assert value_3 == "new_value"

    def test_load_config_handles_nested_structure(self, tmp_path: Path) -> None:
    ...
    def test_load_config_raises_for_missing_file(self, tmp_path: Path) -> None:
    ...
    def test_load_config_raises_for_directory(self, tmp_path: Path) -> None:
    ...
    def test_load_config_raises_for_invalid_yaml(self, tmp_path: Path) -> None:
    ...
    def test_load_config_raises_for_non_dict_root(self, tmp_path: Path) -> None:
...
class TestYamlConfigLoaderGetProperty:
    ...
    @pytest.fixture
    def loader_with_config(self, tmp_path: Path) -> YamlConfigLoader:
    ...
    def test_get_property_simple_key(
        self, loader_with_config: YamlConfigLoader
    ) -> None:
        """Simple dot-notation key retrieves value."""
        name = loader_with_config.get_property("application.name")
        assert name == "Test App"

    def test_get_property_nested_key(
        self, loader_with_config: YamlConfigLoader
    ) -> None:
        """Nested dot-notation key retrieves value."""
        port = loader_with_config.get_property("api.port", cast_type=int)
        assert port == 8000
        assert isinstance(port, int)

    def test_get_property_returns_default_for_missing(
        self, loader_with_config: YamlConfigLoader
    ) -> None:
        """Default value returned for missing key."""
        value = loader_with_config.get_property(
            "nonexistent.key", default_value="default"
        )
        assert value == "default"

    def test_get_property_returns_default_for_null_value(
        self, loader_with_config: YamlConfigLoader
    ) -> None:
    ...
    def test_get_property_casts_to_int(
        self, loader_with_config: YamlConfigLoader
    ) -> None:
    ...
    def test_get_property_casts_to_bool_true(
        self, loader_with_config: YamlConfigLoader
    ) -> None:
        """Boolean true value is handled correctly."""
        enabled = loader_with_config.get_property("api.enabled", cast_type=bool)
        assert enabled is True

    def test_get_property_casts_to_bool_false(
        self, loader_with_config: YamlConfigLoader
    ) -> None:
        """Boolean false value is handled correctly."""
        debug = loader_with_config.get_property("feature_flags.debug", cast_type=bool)
        assert debug is False

    def test_get_property_raises_when_not_loaded(self, tmp_path: Path) -> None:
...
class TestYamlConfigLoaderReload:
    ...
    def test_reload_returns_fresh_config(self, tmp_path: Path) -> None:
