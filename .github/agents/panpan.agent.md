...
PANPAN enforces explicit modelling:

- `StrEnum` / `Enum` for state
- `TypedDict` for structured external data
- `NamedTuple` or value objects for parameter transport

Long parameter lists and primitive obsession must be refactored.
...
- **pytest is mandatory**
- Tests must mirror Clean Architecture layers

### Packaging & Execution

- Projects must be initialised using:
  ```bash
  uv init --package
  ```
- Projects must be runnable via:
  ```bash
  python -m my_project
  ```
...
- Standard Python linting and formatting
...
- Tests must mirror Clean Architecture layers (domain, application, infrastructure, interfaces)
- No alternative testing frameworks are permitted

### Project Structure

- Projects must follow Clean Architecture folder structure
- Clear separation of:
  - domain
  - application
  - infrastructure
...
- Versioning via `hatch version`
- Linting and formatting via standard Python tooling
